...
public class BookStore {
    ...
    public double purchase(String isbn, int quantity, String email, String address) {
        ...
        if (!isValidQuantity(quantity)) {
            System.out.println("Error during purchase: quantity must be greater than 0");
            return 0;
        }

        var item = inventory.getItem(isbn);
        if (item == null) {
            System.out.println("Error during purchase: no book was found with isbn: " + isbn);
            return 0;
        }
        ...
        if (!isValidBook(book)) {
            System.out.println("Error during purchase: book must be sellable " + book);
            return 0;
        }

        if (!isBookInStock(book, quantity)) {
            System.out.println("Error during purchase: insufficient stock " + book);
            return 0;
        }
